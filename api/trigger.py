...
# ==============================================================================
# 1. CONFIGURATION & SECRETS LOADING (Cloud Environment Variables)
# ==============================================================================

# --- Load Secrets from Environment Variables (Set in Vercel Console) ---
# NOTE: All these must be configured as Environment Variables in Vercel settings.
...
def generate_tweet_content() -> Optional[str]:
    ...
    try:
        ...
        # Ensure required text is included, truncated if necessary
...
def post_media_update_to_linkedin():
    ...
    print(f"✅ LinkedIn caption used AS-IS from file.")
    ...
    try:
        ...
        if e.response is not None:
             ...
             if 'Invalid user id' in e.response.text:
                 print(">>> CRITICAL FIX: One or more Instagram usernames in INSTAGRAM_USER_TAGS are invalid.")


# ==============================================================================
# 7. VERCEL ENTRY POINT (Main Execution Handler)
# ==============================================================================

def handler(request):
    """
    Vercel Serverless Function entry point.
    Handles the HTTP request and runs the main automation sequence.
    """
    
    # 1. SECURITY CHECK (CRITICAL)
    # Checks the custom secret key sent in the HTTP header
    if WEB_TRIGGER_KEY:
        provided_key = request.headers.get("X-Trigger-Key")
        if provided_key != WEB_TRIGGER_KEY:
            print(f"🛑 Security Alert: Invalid or missing trigger key.")
            return "Unauthorized: Invalid trigger key.", 403
    ...
    # --- Initial Check for Gemini Key ---
    if not GEMINI_API_KEY:
        print("🛑 ERROR: GEMINI_API_KEY is not configured in Vercel environment.")
        return "Internal Error: API Key Missing.", 500
    ...
    # Post the regular image update
    post_media_update_to_linkedin()

    print("\n" + "=" * 60)

    # Post the Gemini-generated Article (text-only)
    ...
    # Must return a tuple (response_body, status_code) or a Vercel-compatible Response object
    return "Social media automation triggered successfully and completed execution.", 200
